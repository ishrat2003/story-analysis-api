...
```bash
story-survey-api$ sam build --use-container
```
...
```bash
story-survey-api$ sam local invoke HelloWorldFunction --event events/event.json
story-survey-api$ sam local invoke LcFunction --event events/lc.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
story-survey-api$ sam local start-api
story-survey-api$ curl http://localhost:3000/
```
...
```bash
story-survey-api$ sam logs -n HelloWorldFunction --stack-name story-survey-api --tail
```
...
```bash
story-survey-api$ pip install pytest pytest-mock --user
story-survey-api$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name story-survey-api
```
